...
#define AUTO 0xFF // 0-250. Id 255 is reserved for auto initialization of nodeId.
...
public:
	...
	virtual void setAddress(uint8_t address, uint64_t network) = 0;
	// getAddress()
	...
	virtual bool send(uint8_t to, const void* data, uint8_t len, uint8_t pipe = 255) = 0;
	// available(to)
	...
	virtual bool available(uint8_t *to, uint8_t *pipe = NULL) = 0;
	// receive(data)
